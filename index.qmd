...
# Block group centroids — save to processed/ on first run, load thereafter
blockgrp_centroid_path <- file.path(path_proc, "blockgrp_centroid.shp")

if (!file.exists(blockgrp_centroid_path)) {
  blockgrp_centroid <- slco_blockgrp |> st_centroid()
  st_write(blockgrp_centroid, blockgrp_centroid_path, quiet = TRUE)
  message("Block group centroids computed and saved.")
} else {
  blockgrp_centroid <- st_read(blockgrp_centroid_path, quiet = TRUE)
  message("Block group centroids loaded from disk.")
}
...
if (!file.exists(tsfca_path)) {
  ...
  TSFCA_network <- st_read(tsfca_path, quiet = TRUE)
...
if (!file.exists(lisa_path)) {
  ...
  st_write(LISA, lisa_path, quiet = TRUE)
  message("LISA clusters computed and saved.")
} else {
  LISA <- st_read(lisa_path, quiet = TRUE)
